...
- Images: {stats['num_images']} total; synthetic token classes: {', '.join(LABEL_LIST)}.
- Token count: {stats['total_tokens']}
- Class distribution (tokens): {stats['class_counts']}
...
- Splits: Train {len(train_ids)} | Val {len(val_ids)} | Test {len(test_ids)} (70/10/20 approx.).
- LayoutLM: transformer encoder with token classification head; AdamW + linear warmup; batch size {args.layoutlm_batch}; epochs {args.layoutlm_epochs}.
- BiLSTM+CRF: 2x directions LSTM hidden 256, CRF decoding; Adam; batch size {args.bilstm_batch}; epochs {args.bilstm_epochs}.
...
- LayoutLM test accuracy: {_pct(test_acc_layoutlm)}
- BiLSTM+CRF test accuracy: {_pct(test_acc_bilstm)}
